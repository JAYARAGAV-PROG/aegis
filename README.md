...
# Start server — use python -m uvicorn to avoid module path issues
python -m uvicorn main:app --host 0.0.0.0 --port 8000 --reload --app-dir .
```
...
**Vite shows `http proxy error` / `ECONNRESET` for `/api/...`:**

- Start the backend first, then refresh the dashboard. The proxy targets `http://localhost:8000`; if nothing is listening, the connection resets.

**Backend / agent: `httpx.ConnectError` / `WinError 10013` when talking to Supabase:**

- This is Windows blocking outbound HTTPS from Python, or a bad proxy. The backend now forces HTTP/1.1 to Supabase on Windows (avoids some httpx HTTP/2 + Winsock issues).
- Allow **python.exe** outbound in Windows Defender Firewall / your antivirus.
- Try running **uvicorn from a normal (non–Run as administrator) terminal**; some security tools treat elevated processes differently.
- If you use Docker Desktop or WSL2, search for **Hyper-V excluded port ranges** overlapping Windows ephemeral ports (`netsh interface ipv4 show excludedportrange protocol=tcp`).
- If `HTTP_PROXY` / `HTTPS_PROXY` are set globally but invalid, unset them or add to `backend/.env`: `AEGIS_HTTPX_TRUST_ENV=false`.
- After fixing env, hit `http://localhost:8000/api/health` — you should see `"supabase": "connected"`.
